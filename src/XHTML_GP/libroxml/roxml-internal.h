...
typedef struct node {
	...
	unsigned int pos;		/*!< offset of opening node in file */
	unsigned int end;		/*!< offset of closing node in file */
	unsigned int prv;		/*!< internal offset used to keep file position */
 ...
 */
#define PUSH(n)	{ n->prv = *(n->idx); *(n->idx) = n->pos;}
 ...
 * get next char
 */
#define ROXML_FGETC(n)	(n->buf[(*(n->idx))++])
 ...
 * get stream position
 */
#define ROXML_FTELL(n)	(*((int*)n->idx))
 ...
 * set stream position
 */
#define ROXML_FSEEK(n, pos)	{ *((int*)(n)->idx)=(pos); }
 ...
 * get end of stream
 */
#define ROXML_FEOF(n)	(((*(n->idx))>=(strlen(n->buf)-1)))
 ...
 */
#define ROXML_FREAD(b, len, size, n)	{ memcpy(b, n->buf+*(n->idx), (size)*(len)); }
 ...
 * return void
 */
node_t* ROXML_INT roxml_new_node		(int pos, const char * buf, unsigned int * idx);
 ...
 * see roxml_close
 */
node_t*	ROXML_INT roxml_load			(node_t *current_node, const char *buffer);
