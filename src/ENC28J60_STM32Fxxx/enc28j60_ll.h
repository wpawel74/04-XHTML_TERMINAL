 ...
 *
 * ENC28J60 module works with SPI communication with device. For this purpose, library supposes 2, USART based, functions, which are called from ESP module stack when needed:
 *
 * - \ref ENC28J60_LL_SPIInit: Function, which is called when USART should be initialized
...
uint8_t ESP8266_LL_SPIRxTx(uint8_t txbyte){
    //send/receive data via SPI
    TM_SPI_Send( ENC28J60_SPI, data );
    //Return RX byte
...
\endcode
 *
 * \par System Time configuration
 *
 * Set system time when NTP service is enable and time from network was received
 *
\code

void system_set_time( uint32_t time ) {

    //Set sytem time based on time parameter in seconds
}
\endcode
 */
